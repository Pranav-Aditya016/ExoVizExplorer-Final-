...
export const ExoplanetExplorer: React.FC = () => {
  ...
  return (
    <div className="w-full h-screen bg-black relative overflow-hidden">
      {/* Navigation */}
      <nav className="absolute top-0 right-0 z-10 p-4 flex gap-2">
        <Button 
          variant="outline"
          className="bg-white/10 hover:bg-white/20 text-white border-white/20"
          onClick={() => setViewMode(viewMode === 'exoplanets' ? 'solar-system' : 'exoplanets')}
        >
          {viewMode === 'exoplanets' ? 'View Solar System' : 'View Exoplanets'}
        </Button>
        <Link to="/"> 
          <Button 
            variant="outline"
